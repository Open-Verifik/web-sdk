...
export class LivenessDetectionComponent implements OnInit, OnDestroy {
    ...
    constructor(
        ...
        this.faceIdCard = localStorage
            .getItem('idCardFaceImage')
            ?.replace(/^data:.*;base64,/, '');

        if (!this.faceIdCard) {
            alert(
                this._translocoService.translate('id_scanning.face_not_found')
            );
    ...
    listenModeDebug(): void {
        ...
        document.addEventListener('keydown', (event) => {
            if (
                this.debugText.charAt(this.debugIndex) ===
    ...
    setCanvasDimension = () => {
        ...
        this.resizeDimensions = {
            ...
            x:
                this.videoCenterX -
                Math.min(2.8 * this.OVAL.radiusX, this.WIDTH) / 2,
        };

        this.originalDimensions = {
            rectHeight: this.videoDimensions.height,
            rectWidth:
                (this.resizeDimensions.rectWidth / this.WIDTH) *
                this.videoDimensions.width,
            y: 0,
            x:
                (this.resizeDimensions.x / this.WIDTH) *
                this.videoDimensions.width,
        };
    };
            ...
            async () => {
                try {
                    ...
                    if (detection.length > 0) {
                        ...
                        this.isFaceCentered(
                            this.lastFace.landmarks.getNose()[3]
                        );
    ...
    drawFaceAndCenter(detection, ctx): void {
        ...
        if (this.isActiveDebug) {
            ...
            ctx.lineWidth = 4;
            ctx.strokeRect(
                this.videoCenterX - this.marginX,
                this.videoCenterY,
                this.marginX * 2,
                this.marginY * 2
            );
            ...
            const drawBox = new faceapi.draw.DrawBox(box, {
                label: `${detection[0].gender.toUpperCase()} | ${Math.round(
                    detection[0].age
                )} years old`,
            });
    ...
    drawText(ctx, text, x, y): void {
        ...
        ctx.fillRect(
            x - medidasTexto.width / 2,
            y - 30 - padding,
            medidasTexto.width + 2,
            40 + 2 * padding
        );
    ...
    isFaceCentered(nose): void {
        ...
        const faceCenterY =
            (nose.y / this.videoDimensions.height) * this.HEIGHT;
    ...
    isFaceClose(landmarks: faceapi.FaceLandmarks68): void {
        ...
        const totalImageArea = Math.floor(
            this.originalDimensions.rectHeight *
                this.originalDimensions.rectWidth
        );
        ...
        if (
            faceProportion < threshold ||
            landmarks.imageHeight < this.minPixelFace ||
            landmarks.imageWidth < this.minPixelFace
        ) {
            this.errorFace = {
                title: this._translocoService.translate('liveness.get_closer'),
                subtitle: this._translocoService.translate(
                    'liveness.get_closer_subtitle'
                ),
            };
        }
    }
    ...
    captureBase64Image(): void {
        ...
        this.saveImageBase64Intent = setTimeout(() => {
            if (this.errorFace) {
                this.saveImageBase64Intent = clearTimeout(
                    this.saveImageBase64Intent
                );
            }
    ...
    retryLivenessModal(error) {
        const data = {
            ...
            message: this._translocoService.translate(
                'liveness.liveness_error_message',
                { error }
            ),
            icon: {
                show: true,
