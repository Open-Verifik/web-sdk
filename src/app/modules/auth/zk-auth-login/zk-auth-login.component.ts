...
@Component({
	selector: "app-zk-auth-login",
	standalone: true,
	styleUrls: ["./zk-auth-login.component.scss"],
	templateUrl: "./zk-auth-login.component.html",
	imports: [
...
export class ZkAuthLoginComponent implements OnInit, OnDestroy {
	...
	constructor(
		...
		public dialogRef: MatDialogRef<ZkAuthLoginComponent>,
		...
		// Use passed data
		this.project = data?.project || this._passwordlessService.currentProject;
		this.projectFlow = data?.projectFlow || this._passwordlessService.currentProjectFlow;
	...
	async ngOnInit(): Promise<void> {
		// this._splashScreenService.show(); // Don't show global splash for modal
	...
	setMaxDimensions = () => {
		...
		// For modal, we want it large but fitting within dialog
		// The dialog itself is max 95vh/vw (from sign-in activation)
		this.maxHeight = Math.floor(window.innerHeight * 0.7);
	...
	zkLogin(): void {
		...
		this._passwordlessService.zkLogin(payload).subscribe({
			next: (response) => {
			...
			error: (err) => {
				console.error("ZK Login Error", err);
				this.errorContent = err.error || { message: "Authentication Failed" };
				this.showError = true;
			},
			complete: () => {
				this.loadingResults = false;
	...
	retry() {
		thisx.showError = false;
		thisx.errorContent = { message: "" };
		thisx.loadingResults = false;
		thisx.base64Image = null;
		thisx.startAsyncVideo();
	}
}

// Helper for retry callback binding
let thisx: ZkAuthLoginComponent;
